...
func (zt *Z3Translator) VisitBinaryOperation(expr *symbolic.BinaryOperation) interface{} {
	...
	_, isBool := l.(z3.Bool)
	if expr.Operator == symbolic.EQ && isBool {
		return l.(z3.Bool).Eq(r.(z3.Bool))
	}

	_, isArray := l.(z3.Array)
	if expr.Operator == symbolic.EQ && isArray {
		return l.(z3.Array).Eq(r.(z3.Array))
	}

	if expr.Operator == symbolic.NE && isBool {
		return l.(z3.Bool).NE(r.(z3.Bool))
	}

	if expr.Operator == symbolic.NE && isArray {
