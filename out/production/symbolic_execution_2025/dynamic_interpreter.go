	...
	case token.MUL:
		...
		case *symbolic.Ref:
			...
			case symbolic.IntType, symbolic.FloatType, symbolic.BoolType:
				expr = interpreter.Heap.ReadPrimitive(addr)
			case symbolic.ArrayType:
				expr = interpreter.Heap.GetFromArray(addr, indexExpr)
			case symbolic.ObjectType, symbolic.RefType:
				if fieldAddr, ok := e.X.(*ssa.FieldAddr); ok {
...
func (interpreter *Interpreter) interpretDynamicallyStore(e *ssa.Store) []Interpreter {
	...
	case *symbolic.Ref:
		...
		case symbolic.ArrayType:
			interpreter.Heap.AssignToArray(addr, indexExpr, valExpr)
		case symbolic.ObjectType, symbolic.RefType:
